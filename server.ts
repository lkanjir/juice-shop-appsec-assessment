...
function configureApp(app: ReturnType<typeof express>, seq: typeof sequelize) {
  ...
  app.route("/api/Users/:id").get(security.isAuthorized()).put(security.denyAll()).delete(security.denyAll());
  ...
  app.route("/api/Hints/:id").get(security.denyAll()).delete(security.denyAll());
  ...
  app.post("/rest/2fa/verify", rateLimit({ windowMs: 5 * 60 * 1000, max: 100, validate: false }), utils.asyncHandler(twoFactorAuth.verify));
  /* Check 2FA Status for the current User */
  app.get("/rest/2fa/status", security.isAuthorized(), utils.asyncHandler(twoFactorAuth.status));
  /* Enable 2FA for the current User */
  app.post("/rest/2fa/setup", rateLimit({ windowMs: 5 * 60 * 1000, max: 100, validate: false }), security.isAuthorized(), utils.asyncHandler(twoFactorAuth.setup));
  /* Disable 2FA Status for the current User */
  app.post("/rest/2fa/disable", rateLimit({ windowMs: 5 * 60 * 1000, max: 100, validate: false }), security.isAuthorized(), utils.asyncHandler(twoFactorAuth.disable));
...
const uploadToDisk = multer({
  storage: multer.diskStorage({
    ...
    filename: (req: Request, file: any, cb: any) => {
      const name = security.sanitizeFilename(file.originalname).toLowerCase().split(" ").join("-");
